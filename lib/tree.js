...
Tree.prototype = {
  ...
  flatten: function() {
    if (this.isLeaf()){
      ...
      if (typeof(this.interior()) !== "undefined") {
  ...
  // func is: (leftAccumulator, rightAccumulator, value) -> accumulator
  foldUp: function(func, initialAccumulator) {
    if (this.isLeaf()) {
      return func(initialAccumulator, initialAccumulator, this.leaf());
    } else {
      return func(
        this.left().foldUp(func, initialAccumulator),
        this.right().foldUp(func, initialAccumulator),
        this.interior()
      );
    }
  },
  ...
  eq: function(other) {
    if (this.isLeaf()) {
      return other.isLeaf() && this.leaf() === other.leaf();
    } else {
      return !other.isLeaf() &&
        this.interior() === other.interior() &&
        this.left().eq(other.left()) &&
        this.right().eq(other.right());
    }
  }
